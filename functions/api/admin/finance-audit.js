...
export function pdfExport(metadata, report) {
  const lines = [
    'VulcanIQ - Financial Audit & Compliance', `Audit ID: ${metadata.auditId}`, `Generated at: ${metadata.generatedAt}`,
    `Date range: ${metadata.dateRange.from || 'all'} to ${metadata.dateRange.to || 'all'}`, `Generated by: ${metadata.generatedBy}`,
    `Record count: ${metadata.recordCount}`, `Schema version: ${metadata.schemaVersion}`, `Locale: ${metadata.locale}`, `SHA-256: ${metadata.checksum}`, '',
...
export async function onRequest(context) {
  ...
  try {
    ...
    const checksum = await sha256({ dateRange:{from,to}, report });
    const metadata = { auditId, generatedAt, dateRange:{from,to}, generatedBy:auth.user.id, recordCount:Object.values(report.totals).slice(0,5).reduce((sum,value)=>sum+Number(value||0),0), filters:{dateBasis:'finance.entry_date; related records.created_at'}, schemaVersion:'vulcaniq-financial-audit/1', checksum, locale, piiIncluded:false };
    await logAudit(auth.settings,auth.token,auth.user.id,auditId,metadata);
    const filename = `vulcaniq-financial-audit-${from || 'all'}-${to || 'all'}-${auditId}.`;
    if (format === 'csv') return response(200,csvExport(metadata,report),{'Content-Type':'text/csv; charset=utf-8','Content-Disposition':`attachment; filename="${filename}csv"`});
    if (format === 'pdf') return response(200,pdfExport(metadata,report),{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="${filename}pdf"`});
    return response(200,JSON.stringify({metadata}),{'Content-Type':'application/json; charset=utf-8','Content-Disposition':`attachment; filename="${filename}json"`});
  } catch (error) {
