...
Supabase supplies `SUPABASE_URL`, the preferred `SUPABASE_SECRET_KEYS` JSON map, and legacy `SUPABASE_SERVICE_ROLE_KEY` to deployed Edge Functions. The shared runtime also accepts an explicitly configured `SUPABASE_SECRET_KEY`. Confirm the preferred key map or temporary legacy fallback is available before testing; incoming user JWT, webhook-secret, and Cron-secret verification remains unchanged.
