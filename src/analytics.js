...
function isLikelyBot() {
  ...
  return /bot|crawler|spider|crawling|lighthouse|pingdom|headless|preview|facebookexternalhit|whatsapp/i.test(
    navigator.userAgent || ''
  );
}
...
function referrerParts() {
  ...
  if (utm.utm_source) {
    return {
      referrer_domain: '',
      traffic_source: normalizeTrafficSourceValue(utm.utm_source, utm.utm_medium)
    };
  }
  ...
  try {
    ...
    if (domain.includes('facebook.') || domain === 'fb.com' || domain === 'm.facebook.com') {
      return {
        referrer_domain: domain,
    ...
    return {
      referrer_domain: domain.slice(0, 140),
...
function dedupeKey(eventName, payload) {
  if (eventName === 'page_view') {
    return `${eventName}:${payload.section || ''}:${payload.path || ''}:${payload.language || ''}`;
  }

  if (eventName === 'review_view') {
    return `${eventName}:${payload.session_id}`;
  }

  if (eventName === 'excursion_view' || eventName === 'experience_detail_open') {
    return `${eventName}:${payload.session_id}:${payload.metadata?.experience_slug || payload.metadata?.slug || payload.metadata?.experience || ''}`;
  }

  if (eventName === 'experience_card_view') {
    return `${eventName}:${payload.session_id}:${payload.metadata?.experience_slug || payload.metadata?.experience || ''}`;
  }
  ...
  if (eventName === 'booking_form_open') {
    ...
    return [
      eventName,
      payload.session_id,
      meta.experience_slug || meta.experience || meta.request_type || '',
      meta.source_cta || '',
      meta.cta_location || '',
      meta.selected_date || ''
    ].join(':');
...
function buildPayload(eventName, metadata = {}, options = {}) {
  ...
  const section = sanitizeText(
    options.section || cleanMetadata.source_section || cleanMetadata.section || cleanMetadata.location || '',
    80
  );
...
async function sendPayload(payload, options = {}) {
  ...
  for (const endpoint of EVENT_ENDPOINTS) {
    try {
      ...
      // Public analytics must never fall back to anonymous Supabase writes.
      // RLS blocks those writes in production and creates repeated 403 console noise.
      return;
    } catch {
...
function experienceMetadata(experience, metadata = {}) {
  ...
  return {
    ...
    ...(slug ? {
      experience: slug,
      experience_id: metadata.experience_id || slug,
    ...
    ...(slug ? {
      experience_id: metadata.experience_id || slug,
      experience_slug: metadata.experience_slug || slug
    } : {})
  };
}
...
function bookingContext(metadata = {}) {
  return {
    request_type: normalizeRequestType(metadata.request_type) || (metadata.experience_slug || metadata.experience_id ? 'experience' : 'private'),
...
function readFormJourneyStore() {
  ...
  try {
    ...
    Object.entries(parsed || {}).forEach(([key, value]) => {
      if (value?.opened_at_ms && now - Number(value.opened_at_ms) <= FORM_JOURNEY_TTL_MS) {
        output[key] = value;
      }
    });

    if (Object.keys(output).length !== Object.keys(parsed || {}).length) {
      storage.setItem(FORM_JOURNEY_STORAGE_KEY, JSON.stringify(output));
    }
...
function writeFormJourneyStore(store) {
  ...
  try {
    storage.setItem(FORM_JOURNEY_STORAGE_KEY, JSON.stringify(store || {}));
  } catch {}
}
...
export function createFormJourney(formType, metadata = {}) {
  ...
  trackEvent('form_journey_started', journeyMetadata(journey, metadata), {
    dedupe: false
  });
...
export function markFormFieldStarted(journeyId, fieldKey = 'unknown', metadata = {}) {
  ...
  if (firstField) {
    trackEvent('form_field_started', journeyMetadata(journey, {
      ...metadata,
      step_key: fieldKey || metadata.step_key || 'unknown'
    }), {
      dedupe: false
    });
  }
...
export function markFormActivity(journeyId, metadata = {}) {
  ...
  if (metadata.has_selected_experience !== undefined) {
    journey.has_selected_experience = Boolean(metadata.has_selected_experience);
  }

  if (metadata.has_selected_date !== undefined) {
    journey.has_selected_date = Boolean(metadata.has_selected_date);
  }

  if (metadata.has_people_count !== undefined) {
    journey.has_people_count = Boolean(metadata.has_people_count);
  }
...
export function markFormSubmitted(journeyId, metadata = {}) {
  ...
  trackEvent('form_submit_success', journeyMetadata(journey, metadata), {
    dedupe: false
  });
...
export function markFormAbandoned(journeyId, metadata = {}) {
  ...
  if (!journey || journey.submitted_at || journey.abandoned_at || !journey.field_started_at) {
    return null;
  }
  ...
  trackEvent('abandoned_form_detected', journeyMetadata(journey, metadata), {
    dedupe: false,
    transport: 'beacon'
  });
...
export function markFormRecoveredViaWhatsApp(journeyId, metadata = {}) {
  ...
  if (!journey || journey.submitted_at || !journey.abandoned_at || journey.recovered_via_whatsapp_at) {
    return null;
  }
  ...
  trackEvent('abandoned_form_recovered_whatsapp', journeyMetadata(journey, metadata), {
    dedupe: false,
    transport: 'beacon'
  });
