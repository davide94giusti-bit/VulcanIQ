  ...
  return (
    <div className={className} data-experience={experienceId || undefined}>
      <a
        ...
        onClick={(event) => requestContactAttribution(event, { type: 'phone', url: `tel:${contact.phoneTel}`, location, metadata, confirmLabel: contactActionConfirmLabel('phone', lang) })}
      ...
      <a
        ...
        onClick={(event) => requestContactAttribution(event, { type: 'whatsapp', url: whatsappUrl, target: '_blank', location, metadata, confirmLabel: contactActionConfirmLabel('whatsapp', lang) })}
      ...
      <div className="email-action-wrap">
        <button
          ...
          onClick={(event) => requestContactAttribution(event, {
            type: 'email',
            location,
            metadata: { ...metadata, action: 'email_options_open' },
            confirmLabel: contactActionConfirmLabel('email', lang),
            afterConfirm: (selectedMetadata) => { setEmailAttributionMetadata(selectedMetadata || {}); setEmailOpen(true); }
          })}
...
function ContactForm({ lang, formState, setFormState, siteMedia, siteContent, editor }) {
  ...
  const selectedHeardAboutUsMetadata = heardAboutUsMetadata(selectedHeardAboutUs, lang);
  ...
  function update(field, value) {
    if (!trackedFormOpenRef.current.has('field_start')) {
      ...
      }), formState.trackingContext), heardAboutUsMetadata(field === 'heardAboutUs' ? value : formState.heardAboutUs, lang)));
    }
      ...
      <div className="container contact-section-grid">
        ...
        <form className="contact-form" onSubmit={submitRequest}>
          ...
          <div className="request-action-row">
            ...
            <a className="request-action-button request-action-button-secondary" href={`https://wa.me/${contact.phoneWa}?text=${encode(fullMessage)}`} target="_blank" rel="noopener noreferrer" onClick={(event) => requestContactAttribution(event, { type: 'whatsapp', url: `https://wa.me/${contact.phoneWa}?text=${encode(fullMessage)}`, target: '_blank', location: 'booking_modal', metadata: { ...currentTrackingMetadata, source_cta: 'whatsapp_direct' }, defaultSource: formState.heardAboutUs || '', confirmLabel: contactActionConfirmLabel('whatsapp', lang), afterConfirm: (selectedMetadata, source) => setFormState((current) => ({ ...current, heardAboutUs: source || current.heardAboutUs })) })}>{text(lang, 'sendWhatsapp')}</a>
          </div>
        </form>
      </div>
            ...
            <span className="footer-phone-choice">
              ...
              {phoneChoicesOpen && (
                <span className="footer-phone-choice-menu" role="group" aria-label={lang === 'it' ? 'Scegli come contattare Leonardo' : 'Choose how to contact Leonardo'}>
                  <a href={`tel:${contact.phoneTel}`} onClick={(event) => requestContactAttribution(event, { type: 'phone', url: `tel:${contact.phoneTel}`, location: 'footer_phone_menu', metadata: { ...baseMetadata, cta_location: 'footer_phone_menu', source_cta: 'phone_direct' }, confirmLabel: contactActionConfirmLabel('phone', lang) })}>{lang === 'it' ? 'Chiama' : 'Call'}</a>
                  <a href={`https://wa.me/${contact.phoneWa}?text=${encode(message)}`} target="_blank" rel="noopener noreferrer" onClick={(event) => requestContactAttribution(event, { type: 'whatsapp', url: `https://wa.me/${contact.phoneWa}?text=${encode(message)}`, target: '_blank', location: 'footer_phone_menu', metadata: { ...baseMetadata, cta_location: 'footer_phone_menu', source_cta: 'whatsapp_direct' }, confirmLabel: contactActionConfirmLabel('whatsapp', lang) })}>WhatsApp</a>
                </span>
              )}
            </span>
            <br /><a href={buildMailto(contact.email)} onClick={(event) => requestContactAttribution(event, { type: 'email', url: buildMailto(contact.email), location: 'footer', metadata: { ...baseMetadata, source_cta: 'email_direct' }, confirmLabel: contactActionConfirmLabel('email', lang) })}>{contact.email}</a>
          </p>
          <a className="inline-link" href={contact.instagram} target="_blank" rel="noopener noreferrer"><Icon name="insta" />{text(lang, 'instagram')}</a>
        </div>
      </div>
  ...
  return (
    <>
      <div className="mobile-sticky-bar" aria-label="Mobile contact actions">
        <a href={`tel:${contact.phoneTel}`} onClick={(event) => requestContactAttribution(event, { type: 'phone', url: `tel:${contact.phoneTel}`, location: 'sticky_contact_bar', metadata: { ...metadata, source_cta: 'phone_direct' }, confirmLabel: contactActionConfirmLabel('phone', lang) })}><Icon name="phone" />{lang === 'it' ? 'Chiama' : 'Call'}</a>
        <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={(event) => requestContactAttribution(event, { type: 'whatsapp', url: whatsappUrl, target: '_blank', location: 'sticky_contact_bar', metadata: { ...metadata, source_cta: 'whatsapp_direct' }, confirmLabel: contactActionConfirmLabel('whatsapp', lang) })}><Icon name="chat" />WhatsApp</a>
        <a href={emailUrl} onClick={(event) => requestContactAttribution(event, { type: 'email', url: emailUrl, location: 'sticky_contact_bar', metadata: { ...metadata, source_cta: 'email_direct' }, confirmLabel: contactActionConfirmLabel('email', lang), afterConfirm: () => openDefaultEmailApp(contact.email, text(lang, 'emailSubject'), message) })}><Icon name="mail" />Email</a>
      </div>
      {contactAttributionModal}
    </>
  );
...
function heardAboutUsMetadata(value, lang) {
  const clean = normalizeHeardAboutUs(value, { allowAdmin: true });
  if (!clean) return {};
  return { heard_about_us: clean, heard_about_us_label: heardAboutUsLabel(clean, lang) };
}
...
function openResolvedContactAction(action) {
  if (!action) return;
  if (typeof action.afterConfirm === 'function') {
    action.afterConfirm();
    return;
  }
  if (!action.url || typeof window === 'undefined') return;
    ...
    <div className="contact-attribution-backdrop" role="presentation" onClick={onClose}>
      <section className="contact-attribution-modal" role="dialog" aria-modal="true" aria-labelledby="contactAttributionTitle" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="contact-attribution-actions">
          <button className="button primary" type="button" onClick={confirm} disabled={!selectedSource}>{action?.confirmLabel || contactActionConfirmLabel(action?.type, lang)}</button>
...
function useContactAttributionGate(lang) {
  ...
  function confirmAttribution(source) {
    const action = pendingAction;
    if (!action) return;
    const selectedMetadata = heardAboutUsMetadata(source, lang);
    setPendingAction(null);
    trackContactClick(action.type, action.location, { ...(action.metadata || {}), ...selectedMetadata });
    openResolvedContactAction({
      ...action,
      afterConfirm: typeof action.afterConfirm === 'function' ? () => action.afterConfirm(selectedMetadata, source) : undefined
    });
  }
...
function CalendarBookingModal({ lang, request, onClose, onSave }) {
  const [form, setForm] = useState({ status: request.status, requested_date: request.requested_date || '', adults: String(request.adults || ''), children: String(request.children || ''), heard_about_us: request.heard_about_us || 'not_specified', admin_note: request.admin_note || '', decision_note: request.decision_note || '' });
  useBodyScrollLock(true);
  return (
    <div className="modal-backdrop" role="presentation" onClick={onClose}>
      <div className="admin-modal wide booking-edit-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="admin-form-grid booking-edit-modal-grid">
          ...
          <div className="modal-actions full"><button className="button primary" type="button" onClick={() => onSave(request, { ...form, heard_about_us: form.heard_about_us || 'not_specified', heard_about_us_label: heardAboutUsLabel(form.heard_about_us || 'not_specified', lang), adults: Number.parseInt(form.adults || '0', 10), children: Number.parseInt(form.children || '0', 10) })}>{adminCopy(lang, 'Salva modifiche', 'Save changes')}</button><button className="button secondary" type="button" onClick={onClose}>{adminCopy(lang, 'Annulla', 'Cancel')}</button></div>
        </div>
      </div>
    </div>
  );
...
function VisualEditorPreview({ page, setPage, lang, setLang, device, siteMedia, siteContent, editor, setNotice }) {
  const [formState, setFormState] = useState({ language: lang, requestType: 'private', preferredContact: 'whatsapp', partyType: 'solo', adults: '1', children: '0', childrenUnder3Count: '0', heardAboutUs: '', message: text(lang, 'defaultMessage') });
        ...
        <FinanceDetailSection title={adminCopy(lang, 'Cliente', 'Customer')}>
          {booking ? (
            <>
              ...
              <FinanceDetailRow label={adminCopy(lang, 'Fonte conoscenza', 'Discovery source')} value={heardAboutUsLabel(booking.heard_about_us, lang, { fallback: missing })} />
            </>
  ...
  return (
    <article className={`request-card ${compact ? 'compact' : ''}`}>
      ...
      <dl className="request-details-grid">
        ...
        <div><dt>{adminCopy(lang, 'Fonte conoscenza', 'Discovery source')}</dt><dd>{heardAboutUsLabel(request.heard_about_us, lang, { fallback: adminCopy(lang, 'Non disponibile', 'Not available') })}</dd></div>
...
function ManualRequestModal({ lang, session, onClose, onSaved }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      await createManualBookingRequest({ ...form, heard_about_us: cleanHeardAboutUs, heard_about_us_label: heardAboutUsLabel(cleanHeardAboutUs, lang) }, session.user.id);
      onSaved();
    } catch (err) {
    ...
    <div className="modal-backdrop" role="presentation">
      <section className="admin-modal wide" role="dialog" aria-modal="true" aria-labelledby="manualRequestTitle">
        ...
        <form className="admin-form-grid" onSubmit={submit}>
          ...
          <AdminSelect label={text(lang, 'heardAboutUsAdmin')} value={form.heard_about_us || 'not_specified'} onChange={(value) => { update('heard_about_us', value); update('heard_about_us_label', heardAboutUsLabel(value, lang)); }} options={heardAboutUsOptions({ includeAdmin: true }).map((option) => option.value)} formatter={(value) => heardAboutUsLabel(value, lang)} />
...
function App() {
  ...
  const [formState, setFormState] = useState({ language: 'it', requestType: 'private', partyType: 'solo', adults: '1', children: '0', childrenUnder3Count: '0', heardAboutUs: '', message: i18n.it.defaultMessage });
